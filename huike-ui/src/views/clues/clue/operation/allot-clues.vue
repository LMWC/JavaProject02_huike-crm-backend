<template>
  <!-- 分配线索对话框 -->
  <el-dialog title="分配线索" :visible.sync="visible" width="618px" append-to-body :close-on-click-modal="false">
    <div class="tip-box">
      您选择了 <span class="num">{{ form.id.length }}</span> 个线索，请选择人员进行分配
    </div>
...
<script>
import { assignmentClue } from '@/api/clues/clue'
    ...
    submitForm () {
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          this.loading = true
          try {
            const res = await assignmentClue(form)
            if (res.code !== 200) {
              this.loading = false
