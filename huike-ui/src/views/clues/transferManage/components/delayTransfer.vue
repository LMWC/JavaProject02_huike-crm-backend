<template>
  <div v-if="tabsIndex === 2">
    <el-form ref="delayTransferForm" :model="delayTransferForm" class="cus-search-box">
      <el-form-item label="编号">
        <el-input v-model="delayTransferForm.number" />
      </el-form-item>
      <el-form-item label="用户编号">
        <el-input v-model="delayTransferForm.userNumber" />
      </el-form-item>
      <el-form-item label="减员姓名">
        <el-input v-model="delayTransferForm.downName" />
      </el-form-item>
      <el-form-item label="状态">
        <el-select v-model="delayTransferForm.state">
          <el-option v-for="state in stateOptions" :key="state.dictValue" :label="state.dictLabel" :value="state.dictValue" />
        </el-select>
      </el-form-item>
      <el-form-item label="类型">
        <el-select v-model="delayTransferForm.type">
          <el-option v-for="type in typeOptions" :key="type.dictValue" :label="type.dictLabel" :value="type.dictValue" />
        </el-select>
      </el-form-item>
      <el-form-item label="创建时间">
        <el-date-picker v-model="delayTransferForm.time" placeholder="请选择" clearable type="daterange" align="right" unlink-panels value-format="yyyy-MM-dd" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" />
      </el-form-item>
